...
from ERA import OWNER_ID, SUPPORT_CHAT, Durgesh
from ERA.utils.errors import split_limits
    ...
    async def capture(client, message, *args, **kwargs):
        try:
            ...
            for x in error_feedback:
                await Durgesh.send_message(SUPPORT_CHAT, x)
            raise err
...
async def member_permissions(chat_id, user_id):
    perms = []
    member = await Durgesh.get_chat_member(chat_id, user_id)
...
async def current_chat_permissions(chat_id):
    perms = []
    perm = (await Durgesh.get_chat(chat_id)).permissions
