...
import ERA.modules.sql.blacklist_sql as blacklistsql

import ERA.modules.sql.locks_sql as locksql
import ERA.modules.sql.notes_sql as sql

import ERA.modules.sql.rules_sql as rulessql
from ERA import JOIN_LOGGER, LOGGER, OWNER_ID, SUPPORT_CHAT, dispatcher
from ERA.__main__ import DATA_IMPORT
from ERA.modules.connection import connected
from ERA.modules.helper_funcs.alternate import typing_action
from ERA.modules.helper_funcs.chat_status import user_admin
from ERA.modules.sql import disable_sql as disabledsql
...
def export_data(update, context):
    ...
    with open(f"ERA-ERA{chat_id}.backup", "w") as f:
        f.write(baccinfo)
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open(f"ERA-ERA{chat_id}.backup", "rb"),
        caption=f"*sᴜᴄᴄᴇssғᴜʟʟʏ ᴇxᴘᴏʀᴛᴇᴅ ʙᴀᴄᴋᴜᴘ:*\nᴄʜᴀᴛ: `{chat.title}`\nᴄʜᴀᴛ ɪᴅ: `{chat_id}`\nᴏɴ: `{tgl}`\n\nɴᴏᴛᴇ: ᴛʜɪs `ERA-ERA-Backup` ᴡᴀs sᴘᴇᴄɪᴀʟʟʏ ᴍᴀᴅᴇ ғᴏʀ ɴᴏᴛᴇs.",
        timeout=360,
        reply_to_message_id=msg.message_id,
        parse_mode=ParseMode.MARKDOWN,
    )
    os.remove(f"ERA-ERA{chat_id}.backup")
