...
from ERA import DEV_USERS, OWNER_ID, dispatcher, telethn, updater
from ERA.modules.helper_funcs.alternate import typing_action
from ERA.modules.helper_funcs.chat_status import dev_plus
...
def allow_groups(update: Update, context: CallbackContext):
    args = context.args
    if not args:
        state = "off" if ERA.ALLOW_CHATS else "Lockdown is " + "on"
        update.effective_message.reply_text(f"Current state: {state}")
        return
    if args[0].lower() in ["off", "no"]:
        ERA.ALLOW_CHATS = True
    elif args[0].lower() in ["yes", "on"]:
        ERA.ALLOW_CHATS = False
    else:
...
def leave(update: Update, context: CallbackContext):
    if args := context.args:
        ...
        update.effective_message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup(ERA_leave_bt),
        )
