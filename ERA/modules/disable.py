...
from ERA.modules.helper_funcs.handlers import CMD_STARTERS, SpamChecker
...
if is_module_loaded(FILENAME):
    ...
    from ERA.modules.sql import disable_sql as sql
    ...
    def disable_module(update: Update, context: CallbackContext):
        args = context.args
        if len(args) >= 1:
            disable_module = "ERA.modules." + args[0].rsplit(".", 1)[0]
    ...
    def enable_module(update: Update, context: CallbackContext):
        args = context.args
        if len(args) >= 1:
            enable_module = "ERA.modules." + args[0].rsplit(".", 1)[0]
