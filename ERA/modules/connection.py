...
import ERA.modules.sql.connection_sql as sql
from ERA import DEV_USERS, DRAGONS, dispatcher
from ERA.modules.helper_funcs import chat_status
        ...
        if args and len(args) >= 1:
            ...
            if (isadmin) or (isallow and ismember) or (user.id in DRAGONS):
                if connection_status := sql.connect(
                    ...
                    send_message(
                        update.effective_message,
                        f"Successfully connected to *{chat_name}*. \nUse /helpconnect to check ERAilable commands.",
                        parse_mode=ParseMode.MARKDOWN,
                    )
...
def disconnect_chat(update, context):
    if update.effective_chat.type == "private":
        ...
        send_message(update.effective_message, "This command is only ERAilable in PM.")
...
CONN_HELP = """
Actions which are ERAilable with connected groups:-
*User Actions:*
• View Notes
• View Filters
• View Blacklist
