...
import ERA.modules.no_sql.users_db as sql
from ERA import BOT_USERNAME
from ERA import LOGGER as log
from ERA import OWNER_ID, OWNER_USERNAME, SUPPORT_CHAT, TOKEN
from ERA import Durgesh as pbot
from ERA import StartTime, dispatcher, telethn, updater

# needed to dynamically load modules
# NOTE: Module order is not guaranteed, specify that in the config file!
from ERA.modules import ALL_MODULES
from ERA.modules.helper_funcs.chat_status import is_user_admin
from ERA.modules.helper_funcs.decorators import ERAcallback, ERAcmd, ERAmsg
from ERA.modules.helper_funcs.misc import paginate_modules
...
buttons = [
    ...
    [
        InlineKeyboardButton(text="❄ ᴀʙᴏᴜᴛ ❄", callback_data="Durgesh_"),
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module(f"ERA.modules.{module_name}")
...
@ERAcallback(pattern=r"start_back")
@ERAcmd(command="start", pass_args=True)
    ...
    try:
        if mod_match:
            ...
            help_buttons.append(
                [
                    InlineKeyboardButton(text="ʙᴀᴄᴋ", callback_data="help_back"),
                    InlineKeyboardButton(text="sᴜᴘᴘᴏʀᴛ", callback_data="ERA_D_SUPPORT"),
                ]
            )
    ...
    if len(args) >= 2:
        if any(args[1].lower() == x for x in HELPABLE):
            ...
            if isinstance(help_list, list):
                ...
                help_text = help_list
            text = f"ʜᴇʀᴇ ɪs ᴛʜᴇ ᴇʀᴀɪʟᴀʙʟᴇ ʜᴇʟᴘ ғᴏʀ ᴛʜᴇ *{HELPABLE[module].__mod_name__}* ᴍᴏᴅᴜʟᴇ:\n{help_text}"
            help_buttons.append(
                [
                    InlineKeyboardButton(text="ʙᴀᴄᴋ", callback_data="help_back"),
                    InlineKeyboardButton(text="sᴜᴘᴘᴏʀᴛ", callback_data="ERA_D_SUPPORT"),
...
def send_settings(chat_id: int, user_id: int, user=False):
    if user:
        if USER_SETTINGS:
            ...
            dispatcher.bot.send_message(
                user_id,
                "sᴇᴇᴍs ʟɪᴋᴇ ᴛʜᴇʀᴇ ᴀʀᴇɴ'ᴛ ᴀɴʏ ᴜsᴇʀ sᴘᴇᴄɪғɪᴄ sᴇᴛᴛɪɴɢs ᴇʀᴀɪʟᴀʙʟᴇ :'(",
                parse_mode=ParseMode.MARKDOWN,
            )
        ...
        dispatcher.bot.send_message(
            user_id,
            "sᴇᴇᴍs ʟɪᴋᴇ ᴛʜᴇʀᴇ ᴀʀᴇɴ'ᴛ ᴀɴʏ ᴄʜᴀᴛ sᴇᴛᴛɪɴɢs ᴇʀᴀɪʟᴀʙʟᴇ :'(\nsᴇɴᴅ ᴛʜɪs "
            "ɪɴ ᴀ ɢʀᴏᴜᴘ ᴄʜᴀᴛ ʏᴏᴜ'ʀᴇ ᴀᴅᴍɪɴ ɪɴ ᴛᴏ ғɪɴᴅ ɪᴛs ᴄᴜʀʀᴇɴᴛ sᴇᴛᴛɪɴɢs!",
            parse_mode=ParseMode.MARKDOWN,
        )


@ERAcallback(pattern=r"stngs_")
...
@ERAmsg(Filters.status_update.migrate)
...
if __name__ == "__main__":
    log.info(f"[ᴇʀᴀ] →  sᴜᴄᴄᴇssғᴜʟʟʏ ʟᴏᴀᴅᴇᴅ ᴍᴏᴅᴜʟᴇs: {str(ALL_MODULES)}")
