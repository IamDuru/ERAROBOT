...
def get_user_list(__init__, key):
    with open(f"{os.getcwd()}/ERA/{__init__}", "r") as json_file:
        return json.load(json_file)[key]

# enable logging
FORMAT = "[ᴇʀᴀ] %(message)s"
logging.basicConfig(
...
LOGGER = logging.getLogger("[ᴇʀᴀ]")
LOGGER.info("ᴇʀᴀʀᴏʙᴏᴛ ɪs sᴛᴀʀᴛɪɴɢ. ")
...
SPAMWATCH_SUPPORT_CHAT = "ERA_D_SUPPORT"
...
telegraph = Telegraph()
telegraph.create_account(short_name="ERA")
    ...
    if not isinstance(entity, Chat):
        ...
        try:
            ...
            for Durgesh in apps:
                if Durgesh != client:
                    try:
                        entity = await Durgesh.get_chat(entity)
                    except (PeerIdInvalid, ChannelInvalid):
                        pass
                    else:
                        entity_client = Durgesh
                        break
            else:
                entity = await Durgesh.get_chat(entity)
                entity_client = Durgesh
...
from ERA.modules.helper_funcs.handlers import CustomMessageHandler, CustomRegexHandler

tg.RegexHandler = CustomRegexHandler
tg.MessageHandler = CustomMessageHandler

from ERA.modules.helper_funcs.handlers import CustomCommandHandler

if CUSTOM_CMD and len(CUSTOM_CMD) >= 1:
    tg.CommandHandler = CustomCommandHandler

try:
    from ERA.antispam import antispam_cek_user, antispam_restrict_user, detect_user
